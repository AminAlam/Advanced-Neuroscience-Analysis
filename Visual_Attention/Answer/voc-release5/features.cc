...
mxArray *process(const mxArray *mximage, const mxArray *mxsbin) {
  ...
  int blocks[2];
  blocks[0] = (int)Myround((double)dims[0]/(double)sbin);
  blocks[1] = (int)Myround((double)dims[1]/(double)sbin);
  ...
  for (int x = 1; x < visible[1]-1; x++) {
	...
	if (dot > best_dot) {
      ...
      // add to 4 histograms aMyround pixel using linear interpolation
